...
def main(args):
    ...
    checkpoint_fp = '../3DDFA_V2/weights/mb1_120x120.pth' #'models/phase1_wpdc_vdc.pth.tar'
    arch = 'mobilenet_1'
    cfg = yaml.load(open('configs/mb1_120x120.yml'), Loader=yaml.SafeLoader)

    checkpoint = torch.load(checkpoint_fp, map_location=lambda storage, loc: storage)['state_dict']
    model = getattr(mobilenet_v1_v2, arch)(num_classes=62)  # 62 = 12(pose) + 40(shape) +10(expression)
    ...
    for img_fp in args.files:
        ...
        for rect in rects:
            ...
            with torch.no_grad():
                ...
                param = tddfa(input, roi_box)
               
            # 68 pts
            #pts68 = predict_68pts(param, roi_box)
            params.append(param)
            boxes.append(roi_box)
        ...
        if args.dump_pncc:
            ...
            print('Dump to {}'.format(wfp))'''
        if args.dump_res:
            vertices_lst = tddfa.recon_vers(params, boxes, dense_flag=False)
            draw_landmarks(img_ori, vertices_lst, wfp=img_fp.replace(suffix, '_3DDFA.jpg'), show_flg=args.show_flg)
